...
var chunk_KTVZOQTP_exports = {};
__export(chunk_KTVZOQTP_exports, {
...
module.exports = __toCommonJS(chunk_KTVZOQTP_exports);
...
var exists = (0, import_util.promisify)(import_fs2.default.exists);
...
async function formatSchema({ schemaPath: schemaPath2, schema }, inputFormattingOptions) {
  ...
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(
      () => {
        import_chunk_6WHJ22KZ.prismaSchemaWasm.debug_panic();
      },
      { schemaPath: schemaPath2, schema }
    );
  }
  const schemaContent = await (0, import_chunk_5NQFDVEO.$)({ schema, schemaPath: schemaPath2 }).when(isSchemaOnly, async ({ schema: _schema }) => await Promise.resolve(_schema)).when(isSchemaPathOnly, async ({ schemaPath: _schemaPath }) => {
  ...
  const { formattedSchema, lintDiagnostics } = handleFormatPanic(
    () => {
      const formattedSchema2 = formatWasm(JSON.stringify(schemaContent), documentFormattingParams);
      const lintDiagnostics2 = (0, import_chunk_34EIGQR7.lintSchema)({ schema: formattedSchema2 });
      return { formattedSchema: formattedSchema2, lintDiagnostics: lintDiagnostics2 };
    },
    { schemaPath: schemaPath2, schema: schemaContent }
  );
  const lintWarnings = (0, import_chunk_34EIGQR7.getLintWarningsAsText)(lintDiagnostics);
...
function handleFormatPanic(tryCb, { schemaPath: schemaPath2, schema }) {
  try {
    ...
    const panic = new import_chunk_A4QWQE3W.RustPanic(
      ...
      schemaPath2,
      /* schema */
      (0, import_chunk_QKOL7AVL.schemaToStringDebug)(schema)
    );
    throw panic;
  }
}
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_6WHJ22KZ.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function getGenerators(options) {
  ...
  const config = await (0, import_chunk_TZQ6W7Y6.getConfig)({
    datamodel,
  ...
  const dmmf = await (0, import_chunk_GGSJD54R.getDMMF)({
    datamodel,
    datamodelPath: schemaPath,
    prismaPath,
    previewFeatures
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_GGSJD54R.getDMMF)({
            datamodel,
            datamodelPath: schemaPath,
...
async function checkUnsupportedDataProxyMessage(command, args, implicitSchema) {
  ...
  for (const [argName, argValue] of argList) {
    ...
    if (argName.includes("schema")) {
      ...
      const config2 = await (0, import_chunk_TZQ6W7Y6.getConfig)({ datamodel: datamodel2, ignoreEnvVarErrors: true });
      const url = (0, import_chunk_TZQ6W7Y6.resolveUrl)((0, import_chunk_TZQ6W7Y6.getEffectiveUrl)(config2.datasources[0]));
